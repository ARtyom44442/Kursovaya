...
bool Transport::buildRoute(const std::vector<Order>& orders, RouteInfo& route) {
    ...
    for (size_t k = 0; k < orders.size(); ++k) {
        ...
        int bestPriority = 4; // Ищем с меньшим приоритетом (1=VIP, 2=Express, 3=Economy)
        ...
        for (size_t i = 0; i < orders.size(); ++i) {
            ...
            // Сначала доставляем Express (2), потом Economy (3)
            // VIP (1) не должен попадать в групповую доставку, но если попал - он первый
