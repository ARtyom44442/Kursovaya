...
void performGroupDelivery(const std::vector<Transport*>& fleet, std::vector<Order>& groupOrders, int strategy, int virtual_time) {
    ...
    if (!best_route.orderIndices.empty()) {
        best_route.transport->setPosition(filteredOrders[best_route.orderIndices.back()].getDestination());
    }
...
int main() {
    ...
    while (true) {
        ...
        if (choice == 0) {
            ...
            std::cout << "  - Express:  " << simStats.expressOrders << "\n";
            std::cout << "  - Economy:  " << simStats.economyOrders << "\n\n";
